(function(){
  ...
  BeanstalkWorker.prototype.cleanup_job = function(job) {
      var result = {};
      result.type = job[0];
      result.data = job[1];
      return result;
      ...
      return handler(job_data, __bind(function(action, data) {
          ...
          if (!job_canceled) {
            ...
            if (!(typeof action !== "undefined" && action !== null) || ('next' === action)) {
              this.log('Ran job : ' + job_id + ' in ' + duration + 'ms');
              ...
              return this.log('Failed to run job : ' + job_id + ' : ' + action + ' : ' + data);
            }
          }
        }, this));
    } catch (ex) {
